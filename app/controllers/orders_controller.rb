class OrdersController < ApplicationController
  def index
  	#a list of past orders a current user has
  ...
  def new
  	#place an order form
  	@order = Order.new
  	#add basket stuff on this order
  	#override new order with an updated one with basket stuff
  ...
  private
  def order_params
  		params.require(:order).permit(:name, :email, :address_1, :address_2, :city, :country, :postcode, :stripe_token)
  end

  def build_order(o)
  	#add all the session basket stuff to the order itself
  		basket.each do |basket_item|
  			#each basket_item is a product id
